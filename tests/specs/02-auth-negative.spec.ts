...
test.describe('Authentication - Negative Scenarios', () => {
  ...
  test('TC-110: Session should not persist after logout', async ({
    ...
    const user = { ...testData.validUsers[0] };
    user.email = `logout_session.${Date.now()}@automation.com`;
    ...
    // Verify logged in by checking "Account Created" then continue
    ...
    // After continue, it goes to home, possibly with #google_vignette
    // We check for logout link to confirm login
